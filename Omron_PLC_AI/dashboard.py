...
def get_counts():
    ...
    ok = cur.execute("""
        ...
        WHERE tag_name = 'OK Count'
        ORDER BY id DESC LIMIT 1
    """).fetchone()
    ng = cur.execute("""
        SELECT value FROM tag_readings
        WHERE tag_name = 'NG Count'
        ORDER BY id DESC LIMIT 1
    """).fetchone()
    ...
    return {
        ...
        "reject_count": int(ng[0]) if ng else (rejects[0] if rejects else 0)
    }
...
def get_erp_context():
    """Fetch active plan context from ERPNext: WO, JC, workstation, progress."""
    try:
    ...
    try:
        ...
        # active job card (Work In Progress)
        jcs = client.get_doc_list(
            "Job Card",
            filters=[["status", "=", "Work In Progress"]],
            fields=["name", "work_order", "operation", "workstation",
                    "status", "total_time_in_mins", "time_logs"],
            limit=1
        )
        jc_data = jcs["data"] if isinstance(jcs, dict) and "data" in jcs else jcs
        jc = jc_data[0] if jc_data else None
        ...
        if jc:
            context["job_card"] = {
                ...
                "total_time_mins": jc.get("total_time_in_mins"),
            }

            # parent work order
            wo_name = jc.get("work_order")
            if wo_name:
                wo = client.get_doc("Work Order", wo_name)
                wo_doc = wo.get("data", wo) if isinstance(wo, dict) else wo
                context["work_order"] = {
                    "name": wo_doc.get("name"),
                    "item": wo_doc.get("production_item"),
                    "qty": wo_doc.get("qty"),
                    "produced_qty": wo_doc.get("produced_qty"),
                    "status": wo_doc.get("status"),
                }
        else:
            # fall back: most recent non-completed JC (or latest any)
            recent = client.get_doc_list(
                "Job Card",
                fields=["name", "work_order", "operation", "workstation", "status"],
                limit=1,
                order_by="creation desc"
            )
            recent_data = recent["data"] if isinstance(recent, dict) and "data" in recent else recent
            if recent_data:
                jc0 = recent_data[0]
                context["job_card"] = {
                    "name": jc0.get("name"),
                    "workstation": jc0.get("workstation"),
                    "operation": jc0.get("operation"),
                    "status": jc0.get("status"),
                }
                wo_name = jc0.get("work_order")
                if wo_name:
                    wo = client.get_doc("Work Order", wo_name)
                    wo_doc = wo.get("data", wo) if isinstance(wo, dict) else wo
                    context["work_order"] = {
        ...
        return {"erp_connected": False, "job_card": None, "work_order": None}
...
def build_erp_panel(erp):
    if not erp or not erp.get("erp_connected"):
        return """<div class="section">
  <h2>ERP Integration</h2>
  <p style="color:#888">ERPNext unreachable or not configured.</p>
</div>"""

    jc = erp.get("job_card")
    wo = erp.get("work_order")

    jc_rows = ""
    if jc:
        for label, key in [("Job Card", "name"), ("Workstation", "workstation"),
                           ("Operation", "operation"), ("Status", "status"),
                           ("Total Time (min)", "total_time_mins")]:
            jc_rows += f'<div class="config-item"><span class="key">{label}:</span> <span class="val">{html_mod.escape(str(jc.get(key) or "-"))}</span></div>\n'
    else:
        jc_rows = '<p style="color:#888">No Job Card found.</p>'

    wo_rows = ""
    if wo:
        qty = wo.get("qty") or 0
        produced = wo.get("produced_qty") or 0
        pct = (produced / qty * 100) if qty else 0
        status = wo.get("status") or "-"
        color = "green" if status in ("Completed",) else ("yellow" if status in ("Not Started",) else "blue")
        wo_rows += f"""<div class="config-item"><span class="key">Work Order:</span> <span class="val">{html_mod.escape(str(wo.get("name") or "-"))}</span></div>
<div class="config-item"><span class="key">Item:</span> <span class="val">{html_mod.escape(str(wo.get("item") or "-"))}</span></div>
<div class="config-item"><span class="key">Planned Qty:</span> <span class="val">{qty}</span></div>
<div class="config-item"><span class="key">Produced:</span> <span class="val {color}">{produced} ({pct:.1f}%)</span></div>
<div class="config-item"><span class="key">WO Status:</span> <span class="val {color}">{html_mod.escape(status)}</span></div>
<div class="bar-container"><div class="bar-fill" style="width: {pct:.1f}%; background: #4fc3f7;"></div></div>"""
    else:
        wo_rows = '<p style="color:#888">No Work Order linked.</p>'

    return f"""<div class="section">
  <h2>ERP Integration <span class="auto-refresh">(live from ERPNext)</span></h2>
  <div class="grid" style="grid-template-columns: repeat(2, 1fr);">
    <div>
      <h3 style="color:#aaa; font-size:12px; text-transform:uppercase; margin-bottom:10px;">Job Card (Execution)</h3>
      <div class="config-grid" style="grid-template-columns: 1fr;">
{jc_rows}      </div>
    </div>
    <div>
      <h3 style="color:#aaa; font-size:12px; text-transform:uppercase; margin-bottom:10px;">Work Order (Plan)</h3>
      <div class="config-grid" style="grid-template-columns: 1fr;">
{wo_rows}      </div>
    </div>
  </div>
...
<p class="subtitle">Omron NX1P2-9024DT | FINS UDP 192.168.250.7:9600 | 27 tags</p>
...
def api_erp(request):
    erp = get_erp_context()
    if not erp or not erp.get("erp_connected"):
        return JSONResponse({"error": "ERPNext unreachable"}, status_code=503)
    return JSONResponse(erp)
